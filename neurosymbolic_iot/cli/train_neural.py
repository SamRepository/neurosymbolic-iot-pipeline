...
from neurosymbolic_iot.neural_perception.casas_sequence import build_casas_sequences, build_casas_windows_from_raw
...
def casas_collate(batch):
    # batch: [(token_ids, time_deltas, length, y), ...]
    ...
    for i, (tok, td, L, _) in enumerate(batch):
        ...
        token_pad[i, :L] = tok_t
        # normalize time by window length to help stability
        if L > 0:
            td_pad[i, :L] = td_t / max(float(td_t[-1]), 1.0)
...
class SphereTorchDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        y = torch.tensor(self.label2id[w.label], dtype=torch.long)
        return x, y
...
def main() -> int:
    ...
    if args.dataset == "casas":
        ...
        # build windows + split at window-level
        dfw = build_casas_windows_from_raw(cfg, window_minutes=window_minutes, stride_minutes=stride_minutes, min_events=min_events)
        splits = safe_split_df(dfw, train=train_ratio, val=val_ratio, test=test_ratio, seed=seed, stratify_col="label")

        # build sequences + vocab from TRAIN only
        ...
        dfw = pd.DataFrame(
            [{"idx": i, "label": w.label} for i, w in enumerate(windows)]
        )
        splits = safe_split_df(dfw, train=train_ratio, val=val_ratio, test=test_ratio, seed=seed, stratify_col="label")
        ...
        train_w = pick(splits.train)
        val_w = pick(splits.val)
        test_w = pick(splits.test)

        labels = sorted({w.label for w in train_w})
        label2id = {lab: i for i, lab in enumerate(labels)}
        id2label = labels
    ...
    # run-level summary per dataset directory
