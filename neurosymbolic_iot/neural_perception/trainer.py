...
from neurosymbolic_iot.neural_perception.utils import compute_metrics, ensure_dir, save_confusion_csv, save_json
...
def _class_weights(y: np.ndarray) -> torch.Tensor:
    ...
    w = w * (len(y) / np.sum(w * freq))
    # return aligned to class indices 0..C-1 (assumes y already encoded)
    # If y is encoded, classes should be [0..C-1]
...
def evaluate_classifier(model, loader, device: str, id2label: List[str]) -> Tuple[Dict[str, float], np.ndarray, np.ndarray]:
    ...
    # decode to labels for sklearn metrics consistency
...
def train_loop(
    *,
    ...
    # gather train labels to compute weights
    y_train = []
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        val_metrics, yv, pv = evaluate_classifier(model, val_loader, device, id2label) if val_loader else ({}, None, None)
    ...
    # final eval
    val_metrics, yv, pv = evaluate_classifier(model, val_loader, device, id2label) if val_loader else ({}, None, None)
    test_metrics, yt, pt = evaluate_classifier(model, test_loader, device, id2label) if test_loader else ({}, None, None)
